sap.ui.define([
	...
	"use strict";
    return Controller.extend("rrhh.rrhh.controller.CreateEmployee", {
        onBeforeRendering: function() {
            ...
            this._viewCreateEmployee = this.getView();
            var employeeModel = new sap.ui.model.json.JSONModel([]);
            this._viewCreateEmployee.setModel(employeeModel, "employeeModel");      
        },
        goDataEmpleado: function (oEvent) {    	
            let steptwo = this.byId("steptwo");
            let stepone = this.byId("stepone");		
            let button = oEvent.getSource();
            let employeeType = button.data("employeeType");
            ...
            this.employeeModel = this._viewCreateEmployee.getModel("employeeModel");
            this._viewCreateEmployee.getModel("employeeModel").setData({
                _type : employeeType,
                Type : Type,
                _Salary : Salary,
        ...
        validateDNI: function(oEvent){
            ...
            if(this.employeeModel.getProperty("_type") !== "Autonomo"){
                ...
                if (letterList !== letter.toUpperCase()) {
                    this.employeeModel.setProperty("/_DniState","Error");
                }else{
                    this.employeeModel.setProperty("/_DniState","None");
        ...
        onEmployeeValidation: function(oEvent,callback) {
            var object = this.employeeModel.getData();
            var isValid = true;
            if(!object.FirstName){
            ...
            if(isValid) {
                this._wizard.validateStep(this.byId("steptwo"));
            } else {
                this._wizard.invalidateStep(this.byId("steptwo"));
            }
        ...
        wizardCompletedHandler: function(oEvent){
            ...
            this.onEmployeeValidation(oEvent,function(isValid){
                if(isValid){
                    //Se navega a la página review
                    var wizardNavContainer = this.byId("wizardNavContainer");
                    wizardNavContainer.to(this.byId("DataReview"));
                    ...
                    this._viewCreateEmployee.getModel("employeeModel").setProperty("/_numFiles",numFiles);
                    if (numFiles > 0) {
                        var arrayFiles = [];
                        for(var i in files){
                            arrayFiles.push({DocName:files[i].getFileName(),MimeType:files[i].getMimeType()});	
                        }
                        this._viewCreateEmployee.getModel("employeeModel").setProperty("/_files",arrayFiles);
                    }else{
                        this._viewCreateEmployee.getModel("employeeModel").setProperty("/_files",[]);
                    }
                }else{
                    this._wizard.goToStep(this.byId("steptwo"));
                }
            }.bind(this));
        },                
        ...
        onSaveEmployee: function(){
            let jsonDatosEmpleado = this._viewCreateEmployee.getModel("employeeModel").getData();
            let body = {};
            //Se obtienen aquellos campos que no empicen por "_", ya que son los que vamos a enviar
            for (let i in jsonDatosEmpleado) {
                if (i.indexOf("_") !== 0) {
                    body[i] = jsonDatosEmpleado[i];
                }
            }
        ...
        onCancel: function(){
            ...
            sap.m.MessageBox.confirm(this.oView.getModel("i18n").getResourceBundle().getText("sureCancel"),{
